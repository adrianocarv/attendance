...
public class AttendanceLayout extends CssLayout {
	...
	public void enter(AttendanceView parentView, Activity a) {
		...
		labelActivity.setValue(this.getActivityTitle(grid.getSelectedItems().size()));
		fieldTitle.setValue("");
	...
	private String getActivityTitle(int total) {
		String today = DateUtils.isSameDay(Date.valueOf(currentDate), new Date(System.currentTimeMillis())) ? " HJ " : "";
		String title = "" + currentActivity.getName() + ": " + this.getCurrentDateAsString();
		title = total == 0 ? title : "<span style=\"color:#60a0ea;\">" + total + today + "</span> " + title;
		return title;
	}
	...
	private void refreshEditModeComponents() {
		...
		// security
		if (!CurrentUser.isUserInActivityWrite(currentActivity))
			editMode = false;
		...
		if (newDateLayout.isVisible()) {
			...
			if (!fieldNewDate.getValue().equals(currentDate)) {
				...
				if (!newDateList.isEmpty()) {
					new Notification(null, "Alteração não permitida, pois a atividade já tem " + newDateList.size() + " presenças na nova data.", Notification.Type.ERROR_MESSAGE,
